...
- **The plan link is long and therefore fragile to hand-copying.** A plan ID is ~290 characters of base64url, and a single wrong character makes the link unreadable. The app handles this correctly — it fails loudly with *"that plan link is not readable"* rather than executing something wrong — and the **Copy link** button uses the clipboard, so nothing is ever hand-selected. But if a link is retyped or selectively copied, it will break. Verified the hard way: a corrupted copy of a live plan cost an hour of debugging a bug that did not exist.
